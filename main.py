...
class imrt_data(object):
    def __init__(self, workingDir, dataDir):
        ...
        for b in range(self.nBeams):
            Dij = np.fromfile(self.dataDir + 'voxel' + str(self.beams[b]) + '.bin', dtype=np.float32).reshape(
                (self.nDijPB[b], 3))
            self.dij.append(sps.csc_matrix((Dij[:][:, 2], (Dij[:][:, 0] - 1, Dij[:][:, 1] - 1)),
                                           shape=(self.nBPB[b], self.nVox)).transpose())
